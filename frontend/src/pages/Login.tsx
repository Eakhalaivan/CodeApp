...
import { ArrowRight, User, Lock, Facebook, Instagram, Github } from 'lucide-react';
...
export const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast('Welcome back, friend!', 'success');
      navigate('/dashboard');
    } catch (err) {
  ...
  const handleSocialLogin = async (provider: string, email?: string) => {
    try {
      await socialLogin(provider, email);
      addToast(`Signed in with ${provider}!`, 'success');
      navigate('/dashboard');
    } catch (err) {
      addToast(`${provider} login failed`, 'error');
    } finally {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-rose-200">
      ...
      <motion.div 
        ...
        className="w-full max-w-7xl h-[850px] bg-white/10 backdrop-blur-2xl rounded-[4rem] border border-white/20 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col lg:flex-row relative z-10"
      >
        ...
        <div className="hidden lg:flex lg:w-[40%] bg-black relative overflow-hidden p-16 flex-col justify-between">
          ...
          <div className="relative z-10 mt-auto flex items-end justify-between">
            <div className="flex flex-col gap-4">
              <span className="text-white/60 text-sm font-bold">Sign Up</span>
              <button className="px-10 py-4 rounded-full border border-white/20 bg-white/10 backdrop-blur-xl text-white text-sm font-black hover:bg-white/20 transition-all duration-300 active:scale-95 shadow-lg">
                Join Us
              </button>
            </div>
          ...
          <div className="w-full max-w-md">
            <header className="text-center mb-14">
               <motion.h1 
                 ...
                 className="text-6xl lg:text-7xl font-black text-slate-900 mb-4 tracking-[-0.05em]"
               >
                 Hi, Friends
               </motion.h1>
               <motion.p 
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end px-2">
                <button type="button" className="text-rose-500 text-xs font-black hover:opacity-70 transition-opacity uppercase tracking-widest">Forgot password?</button>
              </div>
            ...
            <footer className="mt-14 text-center text-sm font-bold text-slate-400">
               Don't have account? <button onClick={() => navigate('/signup')} className="text-rose-500 hover:underline decoration-2 underline-offset-8 ml-1 font-black transition-all">Sign up</button>
            </footer>
          </div>
        </div>
      </motion.div>
        ...
        {showSocialModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-base text-slate-700 mb-8">to continue to <span className="font-medium">CodeFlux</span></p>

              <div className="w-full border-t border-slate-100">
                {socialAccounts.map((account) => (
                  <button
                    key={account.email}
                    onClick={() => handleSocialLogin(selectedProvider, account.email)}
                ...
                <button
                  onClick={() => { setShowSocialModal(false); /* Focus custom input if needed */ }}
